...
const pageHtml = (s, total) => `<!doctype html><html><head><meta charset='utf-8'><style>${CSS}</style></head><body><div class='slide ${s.theme}' id='s'>${s.extra}<div class='tag'>${s.tag}</div><div class='main' id='main'>${s.main}</div><div class='foot'><span>@BRANDWITHAK</span><span>${s.n}/${total}</span></div></div></body></html>`;

const exists = (p) => access(p).then(() => true, () => false);
const browser = await chromium.launch();
const page = await browser.newPage({ viewport: { width: 1080, height: 1350 } });
for (const f of (await readdir("slides")).filter((x) => x.endsWith(".json"))) {
  const post = f.replace(".json", "");
  const slides = JSON.parse(await readFile(`slides/${f}`, "utf8"));
  await mkdir(`images/${post}`, { recursive: true });
  for (const s of slides) {
    const out = `images/${post}/${String(s.n).padStart(2, "0")}.jpg`;
    if (await exists(out)) continue;
    await page.setContent(pageHtml(s, slides.length), { waitUntil: "networkidle" });
